...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_chechTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="message"]');
...
var accordion = function accordion(triggersSelector) {
  var btns = document.querySelectorAll(triggersSelector); // при каждом клике на определенную кнопку будет менятся клас и спойлер будет показыватся либо скрыватся
  ...
  // });
  // // добавляем клас активности на ту кнопку, которую нажали, а с других кнопок - убираем
  // btns.forEach(btn => {
  //     btn.addEventListener('click', function() {
...
var drop = function drop() {
  var fileInputs = document.querySelectorAll('[name="upload"]'); // формируем массив событий drag&drop и перебираем его
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      var dots;
      var arr = input.files[0].name.split('.'); // для разреления имени картинки

      arr[0].length > 6 ? dots = "..." : dots = "."; // формируем имя картинки
...
var filter = function filter() {
  var menu = document.querySelector('.portfolio-menu'),
      ...
      no = document.querySelector('.portfolio-no'); // фильтрация элементов
  ...
  var typeFilter = function typeFilter(markType) {
    ...
    no.classList.remove('animated', 'fadeIn'); // если передан марк, то будут показыватся элементы, если не был передан аргумент - будет отображено, что таких элементов не существует
  ...
  }; // типовой обработчик для переключение табов


  function toggleMarkType(btn, markType) {
    btn.addEventListener('click', function () {
      typeFilter(markType);
    });
  }

  toggleMarkType(btnAll, markAll);
  toggleMarkType(btnLovers, markLovers);
  toggleMarkType(btnChef, markChef);
  toggleMarkType(btnGirl, markGirl);
  toggleMarkType(btnGuy, markGuy);
  toggleMarkType(btnGranddad, '');
  toggleMarkType(btnGrandmother, ''); // добвляем/убираем клас активности
...
var forms = function forms() {
  var form = document.querySelectorAll('form'),
      ...
      upload = document.querySelectorAll('[name="upload"]'); //сделаем информацию о том, какая картинка была загружена для отправки на сервер
  // валидация инпутов
  ...
  }; // создаем пути, так как разные формы (дизайн или просто вопрос) будут отправляться на разные пути 
  ...
  }; // при загрузке картинки будет отображатся ее сокращенное название + разширение
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots;
      var arr = item.files[0].name.split('.'); // для разреления имени картинки
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // после сабмита будет скрыватся форма и будет показыватся статус отправки

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.parentNode.appendChild(statusMessage); // делаем прозрачную форму с анимацией и через время полностью ее скрываем

      item.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        item.style.display = 'none';
      }, 400); //отображение статуса отправки формы в виде картинки

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); //отображение статуса отправки формы в виде текста

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage); // собираем все данные с формы

      var formData = new FormData(item);
      var api; // поможет сформировать динамический путь, куда будем отправлять форму
      // если форма по иерархии имеет popup-design, то отправляем по одному пути, в ином случае - по другому пути

      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question;
      console.log(api); // обрабатываем запрос

      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (res) {
        console.log(res); // изменяем изображение и текст
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      ...
      // полифил для IE
      var range = elem.createTextRange(); // обьеденяет граничные точки диапазона

      range.collapse(true); // говорим коду, где будет конечная точка выделения

      range.moveEnd('charaster', pos); // говорим коду, где будет начальная точка выделения

      range.moveStart('charaster', pos); // установим курсор и выделем то значение которое сформировалось при помощи moveEnd moveStart
  ...
  function createMask(event) {
    ...
    def = matrix.replace(/\D/g, ''),
        // стат. значение, работает на основе матрици
...
var modals = function modals() {
  var btnPressed = false; // переменная, которая следит за тем, была ли нажата хоть одна кнопка
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        scroll = calcScroll(); // появление модельного окна
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        btnPressed = true; // для значка подарка, который удаляется при клике на него
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display; // перебираем все модельные окна и если хотя бы одно окно открыто, записываем в display block, таким образом переменная display будет true
...
var pictureSize = function pictureSize(imgSelector) {
  var blocks = document.querySelectorAll(imgSelector); // показываем картинки в блоках при наведении при помощи метода slice и скрываем всю информацию внутри блока
...
var scrolling = function scrolling(upSelector) {
  var upElem = document.querySelector(upSelector); // при скроле скрываем или показываем pageup
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      function step(time) {
        ...
        r = toBlock < 0 ? Math.max(widthTop - progress / speed, widthTop + toBlock) : Math.min(widthTop + progress / speed, widthTop + toBlock); // скролим страницу к определенным координатам

        document.documentElement.scrollTo(0, r); // условие когда анимция должна остановится
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); // 1 способ - более простой
  // cards.forEach(card => {
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      ...
      card.classList.add('animated', 'fadeInUp', 'col-sm-3', 'col-sm-offset-0', 'col-xs-10', 'col-xs-offset-1'); // наполняем карточку контентом
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  var items = document.querySelectorAll(slides); // перемещения слайдов вперед/назад
  ...
  showSlides(slideIndex); // вызываем когда кликаем на определенные элементы, в n передаем -1 или 1, тоесть прелистываем назад либо вперед
  ...
  activateAnimation(); // когда пользователь наводит курсор на слайдер, слайдер отключает автоматическое переключение слайдов и включает, когда курсор убирают 
...
}; // для подгрузки элементов с сервера
